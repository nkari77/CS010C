...
HashTable::HashTable (int s) {
    //init size and create new hashtable list of WordEntry items of size "size"
    size = s;
...
int HashTable::computeHash(const string &s) {
    ...
    //for each element in the string, add to sum by casting the char c as an int
    for(char c : s){
...
void HashTable::put(const string &s, int score) {
    ...
    for (WordEntry& entry : hashTable[index]){
        //if the string exists, increase existing score
        if (entry.getWord() == s){
    ...
    //else, push new entry into the list at the given index
...
const double HashTable::getAverage(const string &s) {
    ...
    //else return 2.0 as neutral result
    return 2.0;
}
...
bool HashTable::contains(const string &s) {
    ...
    //for each entry at computed index in the hash table
    for (WordEntry& entry : hashTable[index]){
        //if the given string is found, return true
        if (entry.getWord() == s){
            return true;
        }
    }
